...
('gobject move', 'Syntax: .gobject [modifiers] [$x [$y [$z]]]', 'Move gameobject to character coordinates (or to local coordinates relative to gameobject ($x = forward/backward, $y = left/right, $z) coordinates if they are provided). Modifiers:\n-guid $guid : Target gameobject with $guid instead of selection\n-adeg $deg : Turn gameobject by adding degrees $deg to its current orientation\n-sdeg $deg : Turn gameobject by setting its global orientation in degrees $deg\n-po : Turn gameobject using player orientation', @GMLEVEL_MODERATOR),
