...
('gobject move', 'Syntax: .gobject move [modifiers] [$x [$y [$z]]]', 'Move gameobject to character coordinates (or to local coordinates relative to gameobject ($x = forward/backward, $y = left/right, $z) coordinates if they are provided). Modifiers:\n-guid $guid : Target gameobject with $guid instead of selection\n-adeg $deg : After moving turn gameobject by adding degrees $deg to its current orientation\n-sdeg $deg : After moving turn gameobject by setting its global orientation in degrees $deg\n-po : After moving turn gameobject using player orientation', @GMLEVEL_MODERATOR),
