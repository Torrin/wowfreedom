...
('npc move', 'Syntax: .npc move [modifiers] [$x [$y [$z]]]', 'Move creature to character coordinates (or to local coordinates relative to creature ($x = forward/backward, $y = left/right, $z) coordinates if they are provided). After moving, turn NPC to where character is facing (unless modifiers are used to change this). Modifiers:\n-guid $guid : Target creature with $guid instead of selection\n-adeg $deg : After moving turn creature by adding degrees $deg to its current orientation\n-sdeg $deg : After moving use creature orientation / do not turn creature at all', @GMLEVEL_MODERATOR),
('npc turn', 'Syntax: .gobject turn [modifiers]', 'Set for selected creature orientation same as current character orientation. Modifiers:\n-guid $guid : Target creature with $guid instead of selection\n-adeg $deg : Turn creature by adding degrees $deg to its current orientation\n-sdeg $deg : Turn creature by setting its global orientation in degrees $deg', @GMLEVEL_MODERATOR),
